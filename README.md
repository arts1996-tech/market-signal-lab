...
- デモ・実データ評価共通のOHLCイベントエンジン、流動性・部分約定・値幅制限フラグ・集中リスク・停止規則
...
短期・中期はそれぞれ250万円の独立口座です。暫定仮定は、判断日の次の東証営業日始値で約定、手数料0.10%、スプレッド0.10%、税率0%、100株単位、同時保有2銘柄まで、1銘柄30%までです。1取引の計画損失を初期資金の1%、全保有の計画損失合計を5%以内とし、損切り幅から参考数量を逆算します。前営業日出来高の10%を参加率上限とし、部分約定を許可します。同一日足で利益確定と損切りへ同時到達した場合は損切りを優先します。現金、評価額、実現／未実現損益、最大ドローダウン、残高推移、保有、取引理由、判断カード、監査ID、未見期間別の検証を表示します。合成データの結果であり、実績・予測・投資判断には使用できません。
...
実データ評価用口座の初期設定は、250万円、1銘柄30%上限、同時保有2銘柄、100株単位、手数料0.10%、スプレッド0.10%、税率0%です。前営業日出来高だけを使って参加率上限と部分約定を決め、入力に取引停止・値幅制限フラグがあれば約定不能として扱います。税率は設定可能ですが、実際の税務を再現するものではありません。配当、企業行動、為替、空売り、上場廃止、実際の特別気配・値幅制限データ、動的なスプレッドは未完成であり、実投資判断には使用しません。
