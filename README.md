...
画面だけで確認する場合は、デモモードの画面を開いて「仮想投資評価」タブの「デモ仮想投資を実行」ボタンを押します。短期・中期各250万円の残高と取引履歴が表示されます。
...
候補抽出、短期スクリーニング、仮想評価には、日本株・ETFの重複しない有効な調整済み日次データが30営業日以上必要です。20日指標はこの品質ゲート通過後に利用し、50日・75日移動平均はそれぞれ必要な観測数がそろった銘柄だけで利用します。
