...
このシステムは自動売買や証券会社への発注を行いません。「必ず上がる」「買うべき」といった断定や利益保証をせず、統計的な傾向、根拠、反対材料、過去検証、データ品質を提示します。最終的な投資判断と注文は利用者が行います。2026-08-24に現物、信用買い、信用売り、自動選択を将来の正式要件へ追加しましたが、現在の仮想口座実装は現物買いのみです。信用固有データ、費用、保証金、維持率、期限、強制決済の品質ゲートが完成するまで、下方向シグナルを信用売り利益として扱いません。2026-08-25に、利用者が複数のティッカーを指定し、その集合だけを分析・仮想評価する将来要件も追加しました。分析画面とシミュレーションは分離し、指定集合機能はまだ未実装です。同日、市場の兆候から仮想口座だけで自動取引し、結果レビューを版付きナレッジへ蓄積する将来要件も追加しました。ナレッジ候補は稼働中のルールへ自動反映せず、未見検証と利用者承認後だけ新戦略版へ昇格します。
...
> **現行ToDoの正本:** 今後の最優先事項、実装順序、未完了項目、完了条件は [docs/22_current_priority_todo.md](docs/22_current_priority_todo.md) に一本化しています。過去文書に残る「次の作業」より、この文書を優先します。`NOW-P0-1`〜`NOW-P0-5`、`NOW-P1-1`〜`NOW-P1-9`、`NOW-P2-1`〜`NOW-P2-5`に加え、通常銘柄分析の週次・月次リターン、ATR、相対強度、52週高値乖離を完了しました。次はストキャスティクスです。
...
- 実データ評価用の、次営業日始値約定・買いのみ・資金拘束・同時保有上限・手数料・スプレッド・税率0%の税引前評価を扱うイベント順仮想口座
...
- 判断、見送り、仮想取引結果を振り返り、観測・仮説・検証済み・反証を区別して蓄積する将来のナレッジ更新仕様（現在は未実装）
...
短期・中期はそれぞれ250万円の独立口座です。暫定仮定は、判断日の次の東証営業日始値で約定、手数料0.10%、税率0%の税引前評価、100株単位、同時保有2銘柄まで、1銘柄30%までです。スプレッドと基礎スリッページは前営業日売買代金を10億円以上・2.5億円以上・5千万円以上の3段階に分けた保守的な代理値で、実際の板情報ではありません。5千万円未満は見送ります。1取引の計画損失を初期資金の1%、全保有の計画損失合計を5%以内とし、損切り幅から参考数量を逆算します。前営業日出来高の10%を参加率上限とし、部分約定を許可します。同一日足で利益確定と損切りへ同時到達した場合は損切りを優先します。現金、評価額、実現／未実現損益、最大ドローダウン、残高推移、保有、取引理由、判断カードの時系列、監査ID、複数ベンチマーク、未見期間別の検証を表示します。合成データの結果であり、実績・予測・投資判断には使用できません。
...
実データ評価用口座の初期設定は、250万円、1銘柄30%上限、同時保有2銘柄、100株単位、手数料0.10%、税率0%の税引前評価です。前営業日出来高だけを使って参加率上限と部分約定を決め、前営業日売買代金5千万円未満を除外し、売買代金帯ごとの代理スプレッド／スリッページを適用します。入力に取引停止・値幅制限・特別気配フラグがあれば約定不能として扱いますが、現在の取得元がこれらの実データを常に提供するわけではありません。空売り、実際の板・特別気配・値幅制限データは未完成であり、実投資判断には使用しません。

税の扱いは`tax-accounting-pretax-v1`で固定しています。売却益と配当の税率は0%で、画面、監査manifest、判断カード、取引、前向き台帳に「税引前」「税モデル未実装」を記録します。非ゼロ税率を指定して単純な税引後損益を出す経路は拒否します。口座種別、源泉徴収、損益通算、繰越控除、申告税務は再現していません。将来の税モデルは、対象口座と税務仕様をユーザーが承認した後に別バージョンとして実装します。
...
Mac側DBは`0016_nullable_fx_valuation`まで適用済みです。新規環境では通常どおり次を実行します。
...
確認はブラウザの「システム管理」から、短期・中期口座の「同一期間ベンチマーク比較」を開きます。履歴不足中は比較窓がない理由を表示し、必要履歴到達後は各検証窓の期間、費用前／費用控除後損益率、戦略との差、構成数、欠損理由を表示します。

`local-robustness-diagnostic-v1`は、基準ルールに対して利益確定・損切りを各±10%、スコア閾値を±5、片道手数料を±0.05ポイント、既存の売買代金別スリッページを±25%だけ動かします。一度に変更するのは1項目だけで、基準を含む11通りを同じ登録済み検証窓で比較します。最良条件のランキングや採用は行いません。基準と各変化の双方が3検証窓・30決済以上に達するまでは`not_assessed_small_sample`とし、符号反転または版付き基準を超える損益差だけを脆弱性として表示します。複数条件を同時に見ることで偶然よく見える結果が生じるため、結果からルールを選んだり、同じ検証期間で再調整したりしないでください。「システム管理」→短期／中期口座の「局所感度・頑健性検査」で確認できます。

実データでは既定の`data/validation/live-windows.json`、デモでは`--validation-registry-path /app/data/validation/windows.json`で、シミュレーターを呼ぶ前に口座別の未見期間を登録します。`expanding-window-oos-v2`は、学習期間を拡張しながら完全な20営業日の検証窓だけを評価し、学習入力ハッシュ、検証入力ハッシュ、戦略版、ルールハッシュ、凍結ルールハッシュを保存します。過去シグナルは判断日までの指標と米国市場情報だけで作り、エントリー価格、決済価格、将来リターン、結果ラベルを入力にしません。
...
`phase3-all-assets-v2`では「銘柄・ETF分析」に、東証カレンダー上で完了した週・月の終値リターン、調整済みOHLCによるATR 14と終値比率、完全一致する20営業日の日経平均比、同じ分析日・同業種の他2銘柄以上との相対強度、連続252営業日の調整済み高値に対する52週高値乖離を追加しました。未完了の週・月、欠損した高値・安値、途中欠損のある日経平均、比較銘柄不足、252営業日未満は補完せず`-`と品質警告を表示します。追加指標は既存の注目度・変動候補スコアへ加点しないため、指標追加だけで既存戦略の判断や仮想成績は変わりません。
