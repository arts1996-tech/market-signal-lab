...
LiteはLabと同じDB・サービス層を読み、分析や約定ロジックを複製しません。どちらも証券会社への注文機能を持ちません。Liteの「判断・日米波及」では保存済みの遅延研究記録だけを表示し、「指定ティッカー」では利用者の明示操作により集合の新規版・無効版・分析スナップショットを追記できます。画面の表示や再読み込みだけで分析、シミュレーション、仮想口座は始まりません。

指定ティッカーは1行に`market,exchange,symbol`を入力します（例: `jp,JPX,13060`）。登録済み資産との完全一致だけを許可し、銘柄の追加・除外・並べ替えは既存版を更新せず新版として保存します。分析スナップショットは、保存済み全銘柄分析を選び「分析スナップショットを明示作成」を押した場合だけ作成します。
