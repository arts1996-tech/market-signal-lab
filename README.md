...
- 仮想口座の総損益、最大ドローダウン、勝率、平均取引損益、日経平均の同期間騰落率との差を計算する検証指標
...
短期・中期はそれぞれ250万円の独立口座です。暫定仮定は、判断日の次の東証営業日終値で約定、手数料0.10%、スプレッド0.10%、税率0%、100株単位、同時保有2銘柄まで、1銘柄30%までです。現金、評価額、実現／未実現損益、最大ドローダウン、残高推移、保有、取引理由を表示します。合成データの結果であり、実績・予測・投資判断には使用できません。
...
「仮想投資評価」タブでは実注文を行いません。実データ評価では、対象日の終値を含む情報で判断し、その次の東証営業日の調整済み始値で買い、指定保有日の調整済み終値で決済します。始値が欠損している場合は取引を生成せず、下方向シグナルは空売り利益へ変換せず観察専用にします。買いのみ口座は資金をエントリー時に拘束し、同じ現金の重複利用を防ぎます。

実データ評価用口座の初期設定は、250万円、1銘柄30%上限、同時保有2銘柄、100株単位、手数料0.10%、スプレッド0.10%、税率0%です。税率は設定可能ですが、実際の税務を再現するものではありません。流動性、値幅制限、部分約定、配当、企業行動、為替、空売りは未完成であり、実投資判断には使用しません。
