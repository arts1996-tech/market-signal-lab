...
| 日本株、国内ETFの財務情報 | J-Quants API | 単銘柄入口のみ、Freeでの自動収集は保留 | 2026-08-28の公式比較ではFreeも財務サマリー対象。全銘柄自動収集は認証・クォータ・規約確認まで保留 |
...
## 2026-08-28 J-Quantsプラン比較の再確認

- [J-Quants公式プラン比較](https://jpx-jquants.com/)で、Freeは0円、2年・12週間遅延・5回／分、Lightは月額1,650円、5年・60回／分、Standardは月額3,300円、10年・120回／分、Premiumは月額16,500円、最大20年・500回／分と確認した。金額は税込。
- Lightは日通し株価OHLC、TOPIX、投資部門別、CSVを利用できるが、信用取引残高、空売り関連、TOPIX以外の指数は含まれない。Standardは指数、信用取引残高、空売り関連、指数オプションを含む。Premiumは前場・後場、詳細財務、配当、先物・オプション、売買内訳を含む。
- 分足・ティックは月額5,500円、TDnet／企業開示は月額11,000円の別アドオンであり、導入未承認である。日通しOHLCをリアルタイム価格、板、分足として扱わない。
- 2026-08-16の確認ではFreeの財務情報が対象外と読めたが、2026-08-28の公式比較はFreeも財務サマリー、決算予定を対象としている。古い確認記録は履歴として残し、現行の採用判断では新しい公式比較を優先する。実API応答と契約中プランの権限は契約更新時に再確認する。
- 利用者は上位プランをまだ契約・承認しておらず、契約更新時に再相談する。価格鮮度とTOPIXを優先する場合はLight、信用残・貸借倍率・空売り関連まで進める場合はStandardを最低候補とし、Premiumは現段階で優先しない。
- Standardでも証券会社別の一般信用在庫、金利、貸株料、保証金、返済期限、リアルタイム売建可否がすべて得られるとは限らない。これらはJ-Quants市場データと混在させず、別sourceとして確認する。
