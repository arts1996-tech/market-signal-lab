...
- `selected_universe_forward_activation_events`は指定集合版の日次継続口座を利用者が明示的に有効化・無効化した操作を追記保存する。`request_id`で再試行を冪等化し、UPDATE／DELETEをDB triggerで拒否する。集合表示、分析、過去シミュレーションを有効化操作の代用にしない。
