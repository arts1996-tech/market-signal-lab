...
- データソースの採用判断は `docs/data_sources.md` に記録する。
- J-Quants Free planは日本株・国内ETFの履歴分析候補として扱う。ただしFree planの株価は12週間遅延のため、最新短期判断の根拠として使う場合は画面に明記する。
- J-Quantsの有料プラン、TOPIX四本値、指数四本値、分足、ティック、前場四本値、アドオンはユーザー確認なしで導入しない。
- 欠損日は単純な前方補完で埋めない。
...
- 変動候補タブでは、米国株価指数と日本株価指数の相関、直近の米国指数変動、日本株・ETFの短期指標、仮想投資評価のフィードバックを組み合わせて、大きく動きそうな候補と根拠を表示する。
- 仮想投資評価タブでは実注文を行わず、過去時点で候補に出たと仮定した仮想トレードの損益、エントリー理由、結果理由を表示する。
- 仮想投資の結果は銘柄別に集計し、以後の変動候補抽出スコアへフィードバックする。ただし将来利益の保証や投資助言として扱わない。
