...
- 通常前向き口座、指定集合前向き口座、重い分析は同一ホスト・Dockerコンテナ間で共有する用途別ロックを持ち、多重起動を再試行可能な一時失敗として拒否する。ホスト内ロックをMac／Raspberry Piの単一実行主体判定の代用にしない。
- 指定集合の日次継続口座は、利用者の明示操作を追記保存した有効化イベントがある集合版だけを対象とする。画面表示、分析、集合作成、過去シミュレーションを暗黙の開始条件にしない。
